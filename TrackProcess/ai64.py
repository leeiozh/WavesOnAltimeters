...
fig, axs = plt.subplots(1, 1, figsize=(6, 3), facecolor='w', edgecolor='k')

# чтение треков
track_ship = read_track_radar_ai64('/media/leeiozh/EZH/get_satsFromNC.csv')
print(track_ship)
sat_names = ['j3', 'cfo', 'al', 'h2b', 'h2c','c2', 's3a', 's3b',  's6a']
sat_labels = ['Jason-3', 'CFOSAT', 'SARAL', 'HaiYang-2B',  'HaiYang-2C', 'CryoSat-2', 'Sentinel-3A', 'Sentinel-3B',
              'Sentinel-6A']
sat_data = read_sat_data('/media/leeiozh/EZH/69rae_satdata/', sat_names)

print("tracks uploaded successfully")

# отрисовка карты
station_pos_ll = np.array([wgs84.latlon(ll[0], ll[1]) for ll in track_ship[:, 1:3]])
map = make_map(-40, 40, 50, 10)
#map = make_map(-25, 30, 65, -40)
draw_grid(map, 10, 10)
draw_coords(map, track_lat=[ll.latitude.degrees for ll in station_pos_ll],
            track_lon=[ll.longitude.degrees for ll in station_pos_ll], color='black')
colors = ['orange', 'yellow', 'green', 'red', 'DarkBlue', 'grey', 'LightBlue', 'purple', 'pink']
...
for t in range(len(track_ship[:, 0])):  # цикл по времени
    ...
    for sat_dat in sat_data:  # цикл по спутникам
        ...
        if len(sat_track) > 0:
            print(t + 2, end=' ')
            print(sec_to_utc(track_ship[t, 0]).strftime("%Y-%m-%d %H:%M:%S"), end=' ')
            print('{0: <12}'.format(sat_labels[color_num]), end=' ')
            print('min >>', "{0:3.2f}".format(time[-1]), end=' ; ')
            print('km >>', "{0:3.2f}".format(111 * sat_track[-1][0]), end=' ; ')
            print('SWH >>', sat_track[-1][1])

            # print(str(df["name"].loc[t]), str(sat_labels[color_num]), sat_track[:], time)

plt.savefig('pics/ai64.png', dpi=600)        # , transparent=True
plt.show()
