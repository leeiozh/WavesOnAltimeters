...
for t in range(len(track_ship[:, 3])):  # цикл по времени
    ...
    for sat_dat in sat_data:  # цикл по спутникам
        ...
        for p in range(len(sat_near_ship[0])):  # цикл по точкам в куске траектории, находящемся в окне по времени
            ...
            if near:
                alpha = max(1 - np.abs(sat_near_ship[0, p] - (track_ship[t, 3] + track_ship[t, 4]) / 2) / WIN_TIME, 0.1)
        ...
        if p_min != 0:
            ...
            plt.annotate(str(int(track_ship[:, 0][t])), xy=(track_ship[t, 2], track_ship[t, 1]), xycoords="data", xytext=(30, 0),
                         textcoords='offset points', ha="left", va="center", fontsize=11, bbox=dict(boxstyle="round", facecolor="w",
        ...
        if len(sat_track) > 0:
            # print(sat_track)
            print(sat_names[color_num], int(track_ship[:, 0][t]), converters.sec_to_utc(sat_near_ship[0, p_min]),
                  sat_track, time)

plt.savefig('ai63.png', dpi=1000)
plt.show()
